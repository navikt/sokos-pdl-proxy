...
# Get secret from GCP
AZURE_APP_CLIENT_ID=$(kubectl exec -it $(kubectl get pods | grep sokos-pdl-proxy | cut -f1 -d' ') -c sokos-pdl-proxy -- env | grep -E "AZURE_APP_CLIENT_ID" | cut -d "=" -f 2 | tr -d '\r')
AZURE_OPENID_CONFIG_TOKEN_ENDPOINT=$(kubectl exec -it $(kubectl get pods | grep sokos-pdl-proxy | cut -f1 -d' ') -c sokos-pdl-proxy -- env | grep -E "AZURE_OPENID_CONFIG_TOKEN_ENDPOINT" | cut -d "=" -f 2 | tr -d '\r')
...
echo
echo "(sokos-pdl-proxy) AZURE_APP_CLIENT_ID                      : $AZURE_APP_CLIENT_ID"
